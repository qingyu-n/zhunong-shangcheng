...
<template>
  ...
  >
    ...
    <div
      ...
      class="menu-left relative"
      :class="`menu-left-${getMenuTheme.theme} menu-left-${!menuOpen ? 'close' : 'open'}`"
      :style="{ background: getMenuTheme.background }"
    >
      <!-- 鼠标跟随滑块 (侧边栏) -->
      <div
        class="sidebar-menu-slider"
        :style="sidebarSliderStyle"
      />
...
<script setup lang="ts">
   ...
   * 更新滑块位置到当前悬停的菜单项
   */
  const handleSidebarMouseEnter = (event: MouseEvent) => {
    const target = event.currentTarget as HTMLElement
    if (!target || !sidebarMenuRef.value?.$el) return

    const rect = target.getBoundingClientRect()
    const menuRect = sidebarMenuRef.value.$el.getBoundingClientRect()
  ...
  onMounted(() => {
    // 监听侧边栏菜单容器的鼠标离开事件
    if (sidebarMenuRef.value?.$el) {
      sidebarMenuRef.value.$el.addEventListener('mouseleave', handleSidebarMouseLeave)
    }
  })

  onUnmounted(() => {
    if (sidebarMenuRef.value?.$el) {
      sidebarMenuRef.value.$el.removeEventListener('mouseleave', handleSidebarMouseLeave)
    }
  })
