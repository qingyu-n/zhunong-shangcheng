<template>
  <div class="hot-product-page">
      ...
      <ArtCard title="热销商品管理">
        ...
        <el-table :data="productList" v-loading="loading" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="sales" label="销量" width="80" align="center">
            <template #default="{ row }">
              <el-tag type="danger" size="small">{{ row.sales || 0 }}</el-tag>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="showAddDialog" title="添加热销商品" width="600px">
      <el-form :model="addForm" label-width="100px">
        ...
        <el-form-item label="排序值">
          <el-input-number v-model="addForm.sortOrder" :min="0" :max="999" style="width: 100%" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="showSortDialog" title="修改排序" width="400px">
      <el-form :model="sortForm" label-width="80px">
        <el-form-item label="排序值">
          <el-input-number v-model="sortForm.sortOrder" :min="0" :max="999" style="width: 100%" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const fetchProductList = async () => {
  loading.value = true
  try {
    const data = await adminApi.getHotProductList({
...
const searchProducts = async (query: string) => {
  ...
  try {
    ...
    productOptions.value = list.filter((item: any) => item.isHot !== 1)
  } catch {
...
const handleAdd = async () => {
  ...
  try {
    await adminApi.addHotProduct(addForm.productId)
...
const handleRemove = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要将该商品从热销列表中移除吗？', '提示', { type: 'warning' })
    await adminApi.removeHotProduct(row.id)
...
const handleBatchRemove = async () => {
  try {
    await ElMessageBox.confirm(
      `确定要将选中的 ${selectedRows.value.length} 个商品从热销列表中移除吗？`,
      '批量移除',
      { type: 'warning' }
    )
...
const handleEditSort = (row: any) => {
  sortForm.id = row.id
  sortForm.sortOrder = row.sortOrder || 0
  showSortDialog.value = true
}

const handleUpdateSort = async () => {
  if (!sortForm.id) return
  try {
    await adminApi.updateHotProductSort({ productId: sortForm.id, sortOrder: sortForm.sortOrder })
